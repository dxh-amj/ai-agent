...
export const AgentCard = ({ agent, isPublic = false }: AgentCardProps) => {
  ...
  return (
    <Link
      href={href}
      className="group relative flex flex-col h-full rounded-xl border border-slate-100 bg-white p-6 transition-all duration-300 hover:border-primary/20 hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-1"
    >
      <div className="relative z-10 flex flex-col h-full">
        <div className="flex items-center gap-4 mb-6">
          <AgentAvatar agent={agent} size="md" className="shrink-0" />
          <div className="min-w-0">
            <h3 className="text-lg font-bold text-slate-900 group-hover:text-primary transition-colors duration-300 truncate">
              {agent.name}
            </h3>
